...
Settings::Settings(core::Config* config, core::MainTimer* main_timer,
  ...
  ui_.version_group_box->setTitle("Current version: " + core::kAppFullVersion);
...
void Settings::onUpdateSettingsButtonClicked() {
  ...
  new_settings.is_autoupdates_enabled = ui_.autoupdate_checkbox->isChecked();
  config_->set(std::move(new_settings));
}

void Settings::onCheckUpdatesButtonClicked() { app_updater_->initiateUpdate(); }

void Settings::onUpdatesChecked(bool new_version) {
  ui_.version_check_time->setText(
      new_version ? "New version available!"
                  : "Last check: " + time_provider_->getDateTime().toString(
                                         "dd.MM hh:mm:ss"));
}
